function buy(x) {//点击商品将商品放入购物车
    ...
    var goodsNum = document.getElementById(buyNum).innerText;//！！！获取！！！该模块商品的购买数
    var goodsPrice = document.getElementById(buyPriceNum).innerText;//！！！获取！！！该模块商品的单价
    document.getElementById(buyNum).style.display = "block";//显示购买数的div
    goodsNum++;//购买数+1

    document.getElementById(buyNum).innerText = goodsNum;
    if (goodsNum == 1) {
        var imgId = "goodsImg" + num;// 获取新购买物品的图片Id
        var imgSrc = document.getElementById(imgId).src; //获取新购买物品的图片的地址
        var goodsNameId = "goodsName" + num;//获取商品的名字的Id
        var goodsName = document.getElementById(goodsNameId).innerText;//！！！获取！！！商品的名字
